...
public class JRBXmlMsgBuilder {
    ...
    public String buildXml(List cycleNodeList, ContractInfo contractInfo) throws Exception {

        JRBReqBatchFileUploadMsg jrbReqBatchFileUploadMsg = new JRBReqBatchFileUploadMsg();
        Map map = JRBIntfCodeCfgUtil.getCfgCache(contractInfo.getClass().getName());
        String reqXmlMapping = (String) map.get("reqXmlMapping");
        contractInfo.setRecordCount(cycleNodeList.size() + "");
        contractInfo.setContractInfo(cycleNodeList);
        jrbReqBatchFileUploadMsg.setBody(new JRBReqBody(contractInfo));
        Bean2XmlHandler bean2XmlHandler = new Bean2XmlHandler();
        ...
        try {
            fileWriter = new FileWriter(path + "/" + contractInfo.getBatchNo() + ".xml");
