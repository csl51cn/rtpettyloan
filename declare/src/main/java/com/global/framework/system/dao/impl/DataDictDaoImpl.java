...
public class DataDictDaoImpl extends BaseDaoSupport implements DataDictDao {
	...
	public PageBean queryDictClassForPage(DataDictClass dictClass,
		...
		StringBuffer sql = new StringBuffer("select * from dc_sys_datadictclass where 1=1 ");
	...
	public int deleteDictClass(DataDictClass dictClass) throws BaseException {
		//查询该类别下是否有字典代码数据
		String sql = "select count(1) from dc_sys_datadict b where b.classid=?";
	...
	public List<DataDict> getDataDictList(String code) throws BaseException {
		String sql = "select t.dictid,t.dictcode,t.dictname,t.classid,t.sortno,d.classname from dc_SYS_DATADICT t"
				+ " JOIN dc_Sys_Datadictclass d on t.classid = d.classid"
	...
	public List<ZTreeNode> getDataDictListForZTreeNode() throws BaseException {
		String sql = "select a.classid treeId,'-1' pid, a.classname treeName from dc_sys_datadictclass a order by sortno asc";
