...
public class SendContractIssueInfoBatchFileStrategy implements SendBatchFileStrategy {
    ...
    public ResultModel sendBatchFile(String ids) throws Exception {
        ...
        //上传批量文件
        Map sendContractIssueInfoBatchFileResult = jrbBizInfoDeclareManager.sendContractIssueInfoBatchFile(list);


        //是否有错误信息
        if (sendContractIssueInfoBatchFileResult.get("validateError") != null) {
            //数据校验失败,给出合同信息
            String validateError = (String) sendContractIssueInfoBatchFileResult.get("validateError");
            return ResultModel.fail(validateError);
        } else if (sendContractIssueInfoBatchFileResult.get("error") != null) {
            //批量文件发送失败
            String error = (String) sendContractIssueInfoBatchFileResult.get("error");
            return ResultModel.fail(error);
        }

        String fileName = (String) sendContractIssueInfoBatchFileResult.get("fileName");
        //发送实时报文,指明文件位置
