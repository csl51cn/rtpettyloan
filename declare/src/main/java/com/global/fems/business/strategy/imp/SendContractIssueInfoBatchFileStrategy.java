        ...
        if (StringUtils.equals(declareResult.getDataType(), MessageTypeEnum.ISSUE_INFO.getDesc())) {
            ...
            if (list.size() > 0) {
                if ("200000".equals(declareResult.getDeclareResultCode())) {
                    ...
                    list = list.stream().filter(contractIssueInfo -> contractNoSet.contains(contractIssueInfo.getContractNo())).collect(Collectors.toList());
                    list.forEach(contractInfoCycleNode -> contractInfoCycleNode.setIsSend(0));
                } else {
                    //没有成功,需要将所有的记录设置为未申报
                    list.forEach(contractIssueInfo -> contractIssueInfo.setIsSend(0));
                }
                contractIssueInfoDao.batchUpdateContract(list, false);
            }
        }
    }
}
