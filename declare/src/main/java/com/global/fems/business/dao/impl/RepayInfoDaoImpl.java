...
public class RepayInfoDaoImpl extends BaseDaoSupport implements RepayInfoDao {
    ...
    public RepayInfo findRepayInfoByIdFromBizSys(String id) throws DAOException {
        String sql = "SELECT DISTINCT a.Date_Id AS date_id,a.合同编号 AS contract_no,a.合同编号 AS due_bill_no, b.入账日期 AS repay_date,(CASE a.还款方式 WHEN 1835 THEN b.还款期数 + 1 WHEN 818 THEN " +
                ...
                " CASE dic.Word WHEN '质房贷' THEN a.利率 * 10 * 1.5 WHEN '质房贷B' THEN a.利率 * 10 * 1.5 WHEN '质房贷C' THEN a.利率 * 10 * 1.5 WHEN '质房贷D' THEN a.利率 * 10 * 1.5 WHEN '付易贷' THEN a.利率 * 10 * 1.5  WHEN '车盈贷A' THEN a.利率 * 10 * 1.5 WHEN '车盈贷B' THEN a.利率 * 10 * 1.5 WHEN '车盈贷C' THEN a.利率 * 10 * 1.5 ELSE 20 END, NULL) AS pri_plty_rate,b.逾期天数 AS delay_days, (CASE  a.是否为循环授信贷款   WHEN  870 THEN   '740001'   ELSE  '740002' END ) AS is_real_quota_loan,  ISNULL(g.real_quota_no, '') as real_quota_no  FROM Date_还款登记表 b LEFT JOIN Data_WorkInfo a ON a.Date_Id = b.Date_Id LEFT JOIN " +
                " Data_CompanyInfo c ON a.授信主体编号 = c.Id LEFT JOIN Data_MemberInfo d ON a.授信主体编号 = d.ID LEFT JOIN Date_还款计划表 e ON a.Date_Id = e.Date_Id  LEFT JOIN Dictionary f ON a.授信期限单位 = f.Id Left Join Dictionary As dic On a.产品类别 = dic.Id   LEFT JOIN DC_PETTY_LOAN_CONTRACT g on g.dateid = a.Date_Id and g.islast='Y'  " +
