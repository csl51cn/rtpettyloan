...
public class SpfeCsrDaoImpl extends BaseDaoSupport implements SpfeCsrDao{
	...
	public SpfeCsr findById(String id) throws BaseException {
		String sql = "select t1.*,t2.refno from (select a.*,b.SEQNO primarySeqNo,'000040' tradeNo  from " +
				"  dc_BU_SPFE_CSR a, dc_BU_SPFE_LMT b where a.PRIMARYBIZNO = b.bizno and a.seqno=?" +
				" union  " +
				"  select c.*,d.SEQNO primarySeqNo,'000060' tradeNo  from   dc_BU_SPFE_CSR c, dc_BU_SPFE_MDF d where " +
				" c.PRIMARYBIZNO = d.bizno and c.seqno=? " +
				" union " +
				" select c.*,d.SEQNO primarySeqNo,'000080' tradeNo  from   dc_BU_SPFE_CSR c, dc_BU_SPFE_MKUP d where c.PRIMARYBIZNO = d.bizno and c.seqno=? " +
				") t1, (select * from dc_wfl_taskinfo where txnserialno=? union all select * from dc_wfl_taskinfo_his where txnserialno=? ) t2 where t1.seqno=t2.txnserialno";
	...
	public SpfeCsr findByKey(String txnSerialNo) throws BaseException {
		String sql = "select a.*,b.refNo from dc_bu_spfe_csr a, dc_wfl_taskinfo b where a.seqno=b.txnserialno and a.seqno=?";
