        ...
        if (StringUtils.equals(declareResult.getDataType(), MessageTypeEnum.PAY_PLAN_INFO.getDesc())) {
            ...
            if (list.size() > 0) {
                if ("200000".equals(declareResult.getDeclareResultCode())) {
                    ...
                    list = list.stream().filter(payPlanInfo -> contractNoSet.contains(payPlanInfo.getContractNo())).collect(Collectors.toList());
                    list.forEach(payPlanInfo -> payPlanInfo.setIsSend(0));
                } else {
                    //没有成功,需要将所有的记录设置为未申报
                    list.forEach(contractIssueInfo -> contractIssueInfo.setIsSend(0));
                }
                payPlanInfoDao.batchUpdateContract(list, false);
            }
        }
    }
}
