...
public class PayPlanInfoDaoImpl extends BaseDaoSupport implements PayPlanInfoDao {
    ...
    public List<PayPlanInfo> findPayPlanInfoListByDateIdFromBizSys(String dateId) throws DAOException {
       // String sql = "SELECT a.date_id,a.sign_date,a.contract_no,a.due_bill_no,(CASE c.还款方式 WHEN 1835 THEN  b.计划期数 + 1 WHEN 818 THEN b.计划期数 + 1 ELSE b.计划期数 END )AS counter,b.计划还款日 AS repay_date,b.计划应还本金 AS repay_pri_amt,b.计划应还利息 AS repay_int_amt,DATEADD(m, - 1, b.计划还款日) AS start_date,DATEADD(d, - 1, b.计划还款日) AS end_date FROM DC_CONTRACT_ISSUE_INFO a LEFT JOIN Date_还款计划表 b ON a.date_id = b.Date_Id LEFT JOIN Data_WorkInfo c ON a.date_id = c.Date_Id WHERE b.还款计划类别 = 1212 AND a.date_id = ? ORDER BY counter ";
        String sql = "SELECT a.date_id,a.sign_date,a.contract_no,a.due_bill_no,(CASE c.还款方式 WHEN 1835 THEN  b.计划期数 + 1 WHEN 818 THEN b.计划期数 + 1 ELSE b.计划期数 END )AS counter,b.计划还款日 AS repay_date,b.计划应还本金 AS repay_pri_amt,b.计划应还利息 AS repay_int_amt,(CASE c.还款方式  WHEN 1835 THEN  b.计划还款日 WHEN 818 THEN b.计划还款日 ELSE DATEADD(m, - 1, b.计划还款日) END ) AS start_date," +
                "( CASE c.还款方式 WHEN 1835 THEN  DATEADD(d,-1,DATEADD(m, 1, b.计划还款日)) WHEN 818 THEN DATEADD( d,-1,DATEADD(m,1,b.计划还款日)) ELSE DATEADD(d, - 1, b.计划还款日) END  ) AS end_date FROM DC_CONTRACT_ISSUE_INFO a LEFT JOIN Date_还款计划表 b ON a.date_id = b.Date_Id LEFT JOIN Data_WorkInfo c ON a.date_id = c.Date_Id WHERE b.还款计划类别 = 1212 AND a.date_id = ? ORDER BY counter ";
        return (List<PayPlanInfo>) super.findForListBySql(sql, new Object[]{dateId},PayPlanInfo.class);
    }
